...
from lib.materials import Consts,PDE,PertPNP,Physics
from lib.assembly import FEM,PertFEM
...
pde = PDE(
    c_x = {'is_in_air':[[1.0*Consts.p]],
           'is_in_water':[[80.0*Consts.p]],
           'is_in_solid':[[4.5*Consts.p]]},
...
for i in range(len(height_gap)):
    mesh_prefix = 'capsol/grad2_s{:02.1f}nm'.format(height_gap[i])
    print(mesh_prefix)
    ...
    #load previous simulated results
    ...
    #print(np.sum(pert1.__dict__['ratio:1.00'][66,:,:]
    #             -pert2.__dict__['ratio:1.00'][0,:,:]))

    #find boundary condition for sigma
    sigma = np.zeros((len(fsol),len(mesh.nodes)),dtype=complex)
    for j in range(len(fsol)):
        mask = pert1.mesh.is_on_stern
        func = interp1d(pert1.mesh.nodes[mask,0],fsol[j,mask,-1])

        mask = mesh.is_on_stern
        rho = mesh.nodes[mask,0]
        sigma[j,mask] = func(rho)
